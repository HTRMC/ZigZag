...
pub fn render(
    ...
    const confirm_password_len = state.confirm_password_buffer.len;
    ...
    const confirm_line = blk: {
        const prefix = if (state.field == .confirm_password) "║ ► Confirm:  " else "║   Confirm:  ";
        ...
        const suffix = "║";
        @memcpy(conf_buf[conf_idx..][0..suffix.len], suffix);
        conf_idx += suffix.len;
        break :blk conf_buf[0..conf_idx];
    };
    try center.writeCenteredLine(stdout, offset, confirm_line);
